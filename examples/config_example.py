...
config = {
    "model": {
        "temperature": {"default": 20, "source": {"type": "shell", "command": "echo '25'"}, "effect": {"type": "file_write", "path": temp_path, "template": "Temperature: {{ temperature }}°C\n"}}
    }
}
...
config = {
    "model": {"counter": {"default": 0, "effect": "@logger"}},  # Reference to effects section
    "effects": {"logger": {"type": "file_write", "path": log_path, "template": "Counter: {{ counter }}\n", "append": True}},
}
...
config = {
    "sources": {"date_source": {"type": "shell", "command": "date", "target": "@timestamp"}},  # @ reference to model field
    "model": {"timestamp": {"default": None}, "status": {"default": "idle"}},
    "effects": {"log_timestamp": {"type": "file_write", "path": output_path, "template": "Last update: {{ timestamp }}\n", "source": "@timestamp"}},  # Subscribe to timestamp changes
}
...
config = {
    "model": {
        "value_a": {
            "default": 0,
            # Inline effect
            "effect": {"type": "file_write", "path": file_a, "template": "A: {{ value_a }}"},
        },
        "value_b": {
            "default": 0,
    ...
    "effects": {"shared_logger": {"type": "file_write", "path": file_b, "template": "B: {{ value_b }}"}},
}
...
config = {"model": {"counter": 0, "name": "MyApp", "enabled": True, "tags": ["prod", "v1.0"]}}
